...
- **Diaporama** : sélection des photos importées pour la cérémonie, avec une liste d'ordre dédiée (flèches monter/descendre, retrait) pour corriger une erreur de clic sans devoir tout resélectionner ; réglages de transition (fondu, dissolution, glissement, cut), durée par photo, effet Ken Burns, lecture en boucle.
- **Photo fixe** : possibilité de marquer une photo comme « photo fixe », affichée seule en plein cadre à la place du diaporama — utile en introduction avant que le diaporama ne démarre, ou pour y revenir en cours de cérémonie à la demande de la famille. Bascule en un clic (ou une touche assignable) depuis la régie live, sans perdre la position du diaporama.
- **Photos nature** : pour les familles qui n'ont pas de photo du défunt, ou ne souhaitent pas en diffuser, 24 visuels apaisants générés pour Céréma (pas des photographies — aucune question de droits), répartis en 6 thèmes (ciel, mer, forêt, montagne, terre & champs, lumière) et partagés par toutes les cérémonies. Un thème s'ajoute au diaporama en un clic depuis l'éditeur de cérémonie ; une case « Ordre aléatoire » mélange l'ordre de lecture, pratique quand la famille n'a pas de préférence particulière.
- **Régie live** : écran plein cadre pour le jour de la cérémonie — déroulé cliquable, prompteur (défilement automatique réglable, taille de texte, texte modifiable en direct), lecteur audio avec fondu de sortie et choix de la sortie audio (enceinte Bluetooth, système son de la salle, AirPlay), contrôle du diaporama, chronomètre, compteur d'avance/retard sur le déroulé prévu. Le sélecteur de sortie audio s'adapte automatiquement à l'appareil : menu déroulant direct sur Chrome/Edge/Android, bouton AirPlay natif sur iPhone/iPad/Safari. Un onglet « Bibliothèque » dans le panneau Musique permet de parcourir et rechercher les musiques disponibles pour cette cérémonie (bibliothèque libre de droit + musiques importées pour elle), et de lancer directement un morceau en un clic sans quitter la régie.
...
- **Bibliothèque de poèmes** : une dizaine de poèmes de grands auteurs (Hugo, Ronsard, Baudelaire, Verlaine, Nerval, Musset, Sully Prudhomme, Chénier, Scott Holland…), tous dans le domaine public, classés par situation de deuil (mort d'un enfant ou d'une personne jeune, deuil périnatal, mort brutale ou accidentelle, après un suicide, grand âge, séparation, grands classiques) avec une note d'adaptation pour chacun, à insérer dans une étape en un clic.
